...
const AudioWaveformComponent: React.FC<AudioWaveformProps> = ({ 
  ...
  const barCount = 32;
  const barValues = Array(barCount).fill(0).map(() => useSharedValue(0.3));
  
  // Gradient colors from blue to green to purple
  const gradientColors = [
    '#4158D0', '#4B5EE8', '#5564FF', '#5F6CFF', '#6975FF', '#737EFF', '#7D87FF', '#8790FF',
    '#90A0FF', '#8BAFFF', '#86BEFF', '#80CDFF', '#7ADCFF', '#74EBFF', '#6EFAFF', '#68FFFC',
    '#60F5F0', '#58F0E0', '#50EBD0', '#48E6C0', '#40E1B0', '#38DCA0', '#30D790', '#29D280',
    '#21CD70', '#41D46C', '#61DB68', '#81E264', '#A1E960', '#B0ED5C', '#C0F158', '#D0F554',
    '#C8F050', '#BEC24C', '#B49448', '#AA6644', '#A03840', '#963A60', '#8C3C80', '#823EA0',
    '#7840C0', '#7E4CD8', '#8458F0', '#8A64FF', '#9070FF', '#967CFF', '#9C88FF', '#4158D0'
  ];
  ...
  useEffect(() => {
    ...
    barValues.forEach((bar, index) => {
      const centerEffect = 1 - Math.abs((index - barCount/2) / (barCount/2)) * 0.5;
      const randomHeight = (Math.random() * 0.6 + 0.2) * baseIntensity * centerEffect;
        ...
        {barValues.map((_, index) => (
          <Animated.View 
            ...
            style={[
              ...
              { 
                backgroundColor: gradientColors[(index + colorOffset) % gradientColors.length],
              }
            ]}
          />
        ))}
